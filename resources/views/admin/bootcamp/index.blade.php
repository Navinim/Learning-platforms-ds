                    ...
                    <section class="card">
                        <div class="card-body">
                            <header class="card-header d-flex justify-between">
                                ...
                                <a href="{{ route('bootcamps.create') }}" class="btn btn-success"
                                    ...
                                    <tbody>
                                        @foreach ($bootcamp as $item)
                                            <tr class="gradeX">
                                                ...
                                                <td>{{ $item->title}}</td>
                                                <td>
                                                        <img src="{{ asset('uploads/bootcamps/'. $item->banner) }}" style="height: 150px;width:200px"/>
                                                        
                                                </td>
                                                <td>
                                                    <a href="{{ route('bootcamps.edit', $item->id) }}"
                                                        class="btn btn-primary mx-2"><i class="bx bx-edit"></i></a>

                                                    <form method="POST"
                                                        action="{{ route('bootcamps.destroy', $item->id) }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button
